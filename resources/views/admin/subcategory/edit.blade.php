            ...
            <div class="card-body">
                <form method="POST" action="{{ route('admin.subcategory.update', $subcategory->id) }}" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="videoName" name="name" value="{{ $subcategory->name }}" class="form-control" style="border: 1px solid #ced4da; border-radius: 0.25rem;">
                    </div>
                    <div class="form-group">
                        <label for="videoStatus">Status</label>
                        <select id="videoStatus" name="status" class="form-control custom-select">
                            <option selected disabled>Select Status</option>
                            <option value="published" @if($subcategory->status == 'published'){{'selected'}}@endif>Published</option>
                            <option value="draft" @if($subcategory->status == 'draft'){{'selected'}}@endif>Draft</option>
                        </select>
                    </div>
                    <div class="row">
                        <div class="col-12">
